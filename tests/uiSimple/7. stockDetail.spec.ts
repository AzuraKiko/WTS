...
test.describe("Stock Detail Tests", () => {
    ...
    test("TC_001: Check stock detail page", async ({ page }) => {
        // Check stock detail of stock code
        ...
        // Check stock detail of derivative code
        await menu.openMenuHeader("Phái sinh");
        const { stockCode: derivativeStockCode } = await stockDetailPage.openFromPriceBoardFirstRow(stockDetailPage.derivativeModal);
        ...
        await stockDetailPage.expectSymbolDerivativeMatched(derivativeStockCode);

        await stockDetailPage.expectDerivativeChartVisible();

        const timeframeCurrentDerivative = await chartPage.getTimeframeCurrent();

        const symbolCurrentDerivative = await chartPage.getCurrentSymbol();
        expect(symbolCurrentDerivative).toBe(derivativeStockCode);

        const chartResultDerivative = await chartHasDataPipeline(page, {
            symbol: derivativeStockCode,
            timeframe: timeframeCurrentDerivative,
        }, {
            chartLocator: chartPage.chartLocator
        });

        expect(chartResultDerivative.hasData).toBeTruthy();
        // console.log('chartResult', chartResult.evidence);
        // console.log('chartResult bestZoom', chartResult.bestZoom);
